...
export default function Wrapper({
  ...
  const [{ canDrop, isOver }, dropRef] = useDrop({
    ...
    drop: (item, monitor) => {
      ...
      console.log(item.dragItem, 'items');
  ...
  if (isSelected) {
    overwriteStyle = {
      ...overwriteStyle,
      outline: $id !== '#' && '2px solid #409eff', // 最外层容器不需要边框
      borderColor: '#fff',
      zIndex: 5,
    };
  }
  ...
  return (
    ...
    >
      {!inside && isSelected && $id !== '#' && !hiddenControls && (
        <div
          style={{
            position: 'absolute',
            top: -2,
      ...
      {isSelected && !inside && $id !== '#' && !hiddenControls && (
        <div
          style={{
            position: 'absolute',
            zIndex: 20,
